...
/**
 * Set the mail digest option in a specific foreact for a user.
 *
...
$foreact = $DB->get_record('foreact', array('id' => $id));
...
$url = new moodle_url('/mod/foreact/maildigest.php', array(
    'id' => $id,
...
$digestoptions = foreact_get_user_digest_options();

$info = new stdClass();
$info->name  = fullname($USER);
$info->foreact = format_string($foreact->name);
foreact_set_user_maildigest($foreact, $maildigest);
...
if ($maildigest === -1) {
    ...
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
        'mod_foreact', $info);
    $updatemessage = get_string('emaildigestupdated_default', 'foreact', $info);
} else {
    $info->maildigesttitle = $digestoptions[$info->maildigest];
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
        'mod_foreact', $info);
    $updatemessage = get_string('emaildigestupdated', 'foreact', $info);
}
