...
require_once($CFG->dirroot . '/mod/foreact/tests/generator/lib.php');
require_once($CFG->dirroot . '/mod/foreact/lib.php');

/**
 * Provides the unit tests for foreact search.
 *
 * @package     mod_foreact
 * @category    test
 * @copyright   2015 David Monllao {@link http://www.davidmonllao.com}
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_foreact_search_testcase extends advanced_testcase {

    /**
     * @var string Area id
     */
    protected $foreactpostareaid = null;

    public function setUp() {
        $this->resetAfterTest(true);
        set_config('enableglobalsearch', true);

        $this->foreactpostareaid = \core_search\manager::generate_areaid('mod_foreact', 'post');
    ...
    public function test_search_enabled() {
        ...
        $searcharea = \core_search\manager::get_search_area($this->foreactpostareaid);
    ...
    public function test_posts_indexing() {
        ...
        $searcharea = \core_search\manager::get_search_area($this->foreactpostareaid);
        $this->assertInstanceOf('\mod_foreact\search\post', $searcharea);
    ...
    public function test_posts_document() {
        ...
        set_coursemodule_visible($foreact2->cmid, 0);
        ...
        $post1 = $DB->get_record('foreact_posts', array('id' => $discussion1reply1->id));
        $post1->foreactid = $foreact1->id;
        $post1->courseid = $foreact1->course;

        $doc = $searcharea->get_document($post1);
        $this->assertInstanceOf('\core_search\document', $doc);
        $this->assertEquals($discussion1reply1->id, $doc->get('itemid'));
        $this->assertEquals($this->foreactpostareaid . '-' . $discussion1reply1->id, $doc->get('id'));
    ...
    public function test_posts_access() {
        ...
        $record->userid = $user1->id;
        $record->message = 'post2';
        $discussion2reply1 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_post($record);
    ...
    public function test_attach_files() {
        ...
        $post = $DB->get_record('foreact_posts', array('discussion' => $discussion1->id));
        $filerecord = array(
            'contextid' => context_module::instance($foreact1->cmid)->id,
