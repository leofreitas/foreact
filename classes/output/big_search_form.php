...
class big_search_form implements renderable, templatable {
    ...
    public function __construct($course) {
        ...
        $this->actionurl = new moodle_url('/mod/foreact/search.php');

        $foreactoptions = ['' => get_string('allforeacts', 'foreact')] + foreact_menu_list($course);
        $this->foreactoptions = array_map(function($option) use ($foreactoptions) {
            return [
                'value' => $option,
                'name' => $foreactoptions[$option]
            ];
        }, array_keys($foreactoptions));
    }
    ...
    /**
     * foreact ID setter search criteria.
     *
     * @param int $foreactid The foreact ID.
     */
    public function set_foreactid($foreactid) {
        $this->foreactid = $foreactid;
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $tagtypestoshow = \core_tag_area::get_showstandard('mod_foreact', 'foreact_posts');
        ...
        $data->tagsenabled = \core_tag_tag::is_enabled('mod_foreact', 'foreact_posts');
        $namefield = empty($CFG->keeptagnamecase) ? 'name' : 'rawname';
        $tags = $DB->get_records('tag',
            array('isstandard' => 1, 'tagcollid' => \core_tag_area::get_collection('mod_foreact', 'foreact_posts')),
        ...
        if ($this->foreactid && !empty($this->foreactoptions)) {
            foreach ($this->foreactoptions as $index => $option) {
                if ($option['value'] == $this->foreactid) {
                    $this->foreactoptions[$index]['selected'] = true;
                } else {
                    $this->foreactoptions[$index]['selected'] = false;
                }
            }
        }
        $data->foreactoptions = $this->foreactoptions;
