...
/**
 * This file contains a custom renderer class used by the foreact module.
 *
 * @package   mod_foreact
 ...
 * is used by the foreact module.
 *
 * @package   mod_foreact
 * @copyright 2009 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 **/
class mod_foreact_renderer extends plugin_renderer_base {
    ...
    public function neighbouring_discussion_navigation($prev, $next) {
        ...
        if ($prev || $next) {
            ...
            if ($prev) {
                $url = new moodle_url('/mod/foreact/discuss.php', array('d' => $prev->id));
                $html .= html_writer::start_tag('li', array('class' => 'prev-discussion'));
                $html .= html_writer::link($url, format_string($prev->name),
                    array('aria-label' => get_string('prevdiscussiona', 'mod_foreact', format_string($prev->name))));
                $html .= html_writer::end_tag('li');
            }
            if ($next) {
                $url = new moodle_url('/mod/foreact/discuss.php', array('d' => $next->id));
                $html .= html_writer::start_tag('li', array('class' => 'next-discussion'));
                $html .= html_writer::link($url, format_string($next->name),
                    array('aria-label' => get_string('nextdiscussiona', 'mod_foreact', format_string($next->name))));
                $html .= html_writer::end_tag('li');
            }
     ...
     * @return string
     */
    public function subscriber_overview($users, $foreact , $course) {
        $output = '';
        $modinfo = get_fast_modinfo($course);
        if (!$users || !is_array($users) || count($users)===0) {
            $output .= $this->output->heading(get_string("nosubscribers", "foreact"));
        } else if (!isset($modinfo->instances['foreact'][$foreact->id])) {
            ...
            $strparams->name = format_string($foreact->name);
            $strparams->count = count($users);
            $output .= $this->output->heading(get_string("subscriberstowithcount", "foreact", $strparams));
            $table = new html_table();
            $table->cellpadding = 5;
            $table->cellspacing = 5;
    ...
    public function subscribed_users(user_selector_base $existingusers) {
        ...
        $output .= html_writer::tag('p', get_string('forcesubscribed', 'foreact'));
    ...
    public function timed_discussion_tooltip($discussion, $visiblenow) {
        ...
        if ($discussion->timestart) {
            $dates[] = get_string('displaystart', 'mod_foreact').': '.userdate($discussion->timestart);
        }
        if ($discussion->timeend) {
            $dates[] = get_string('displayend', 'mod_foreact').': '.userdate($discussion->timeend);
        }

        $str = $visiblenow ? 'timedvisible' : 'timedhidden';
        $dates[] = get_string($str, 'mod_foreact');
    ...
    /**
     * Display a foreact post in the relevant context.
     *
     * @param \mod_foreact\output\foreact_post $post The post to display.
     * @return string
     */
    public function render_foreact_post_email(\mod_foreact\output\foreact_post_email $post) {
        $data = $post->export_for_template($this, $this->target === RENDERER_TARGET_TEXTEMAIL);
        return $this->render_from_template('mod_foreact/' . $this->foreact_post_template(), $data);
    }
    ...
    /**
     * Create the inplace_editable used to select foreact digest options.
     *
     * @param   stdClass    $foreact  The foreact to create the editable for.
     * @param   int         $value  The current value for this user
     * @return  inplace_editable
     */
    public function render_digest_options($foreact, $value) {
     ...
     *
     * @param \mod_foreact\output\quick_search_form $form The renderable.
     * @return string
     */
    public function render_quick_search_form(\mod_foreact\output\quick_search_form $form) {
        return $this->render_from_template('mod_foreact/quick_search_form', $form->export_for_template($this));
    }

    /**
     * Render big search form.
     *
     * @param \mod_foreact\output\big_search_form $form The renderable.
     * @return string
     */
    public function render_big_search_form(\mod_foreact\output\big_search_form $form) {
        return $this->render_from_template('mod_foreact/big_search_form', $form->export_for_template($this));
    }
}
