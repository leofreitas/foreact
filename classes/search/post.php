...
class post extends \core_search\base_mod {
    ...
    /**
     * Returns recordset containing required data for indexing foreact posts.
     *
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        $sql = 'SELECT fp.*, f.id AS foreactid, f.course AS courseid
                  FROM {foreact_posts} fp
                  JOIN {foreact_discussions} fd ON fd.id = fp.discussion
    ...
    public function get_document($record, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('foreact', $record->foreactid, $record->courseid);
    ...
    /**
     * Add the foreact post attachments.
     *
    ...
    public function attach_files($document) {
        ...
        $cm = $this->get_cm('foreact', $post->foreact, $document->get('courseid'));
        $context = \context_module::instance($cm->id);

        // Get the files and attach them.
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_foreact', 'attachment', $postid, "filename", false);
    ...
    public function check_access($id) {
        ...
        try {
            ...
            $foreact = $this->get_foreact($post->foreact);
            $discussion = $this->get_discussion($post->discussion);
            $cminfo = $this->get_cm('foreact', $foreact->id, $foreact->course);
    ...
    /**
     * Link to the foreact post discussion
     *
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/foreact/discuss.php', array('d' => $post->discussion));
    }
    ...
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/foreact/view.php', array('id' => $contextmodule->instanceid));
    }

    /**
     * Returns the specified foreact post from its internal cache.
     *
     * @throws \dml_missing_record_exception
     * @param int $postid
     * @return stdClass
    ...
    protected function get_post($postid) {
        if (empty($this->postsdata[$postid])) {
            $this->postsdata[$postid] = foreact_get_post_full($postid);
            if (!$this->postsdata[$postid]) {
                throw new \dml_missing_record_exception('foreact_posts');
            }
        }
    ...
    /**
     * Returns the specified foreact checking the internal cache.
     *
     ...
     * @return stdClass
     */
    protected function get_foreact($foreactid) {
        global $DB;

        if (empty($this->foreactsdata[$foreactid])) {
            $this->foreactsdata[$foreactid] = $DB->get_record('foreact', array('id' => $foreactid), '*', MUST_EXIST);
        }
        return $this->foreactsdata[$foreactid];
    }
    ...
    protected function get_discussion($discussionid) {
        ...
        if (empty($this->discussionsdata[$discussionid])) {
            $this->discussionsdata[$discussionid] = $DB->get_record('foreact_discussions',
