@mod @mod_foreact
Feature: Teachers can edit or delete any foreact post
  In order to refine the foreact contents
  As a teacher
  I need to edit or delete any user's foreact posts
  ...
  Background:
    ...
    And I reply "Teacher post subject" post from "Test foreact name" foreact with:
  ...
  Scenario: A teacher can delete another user's posts
    ...
    And I click on "Delete" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][contains(., 'Student post subject')]" "xpath_element"
  ...
  Scenario: A teacher can edit another user's posts
    ...
    And I follow "Test foreact name"
    And I follow "Teacher post subject"
    And I click on "Edit" "link" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][contains(., 'Student post subject')]" "xpath_element"
  ...
  Scenario: A student can't edit or delete another user's posts
    ...
    Then I should not see "Edit" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][contains(., 'Teacher post subject')]" "xpath_element"
    And I should not see "Delete" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][contains(., 'Teacher post subject')]" "xpath_element"
