...
/**
 * A Handler to process replies to foreact posts.
 *
 * @package    mod_foreact
...
namespace mod_foreact\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_foreact');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_foreact');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('foreact_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $cm = get_fast_modinfo($course->id)->instances['foreact'][$foreact->id];
        ...
        if (!$canpost) {
            $data = new \stdClass();
            $data->foreact = $foreact;
            throw new \core\message\inbound\processing_failed_exception('messageinboundnopostforeact', 'mod_foreact', $data);
        }
        ...
        if (!\core_availability\info_module::is_user_visible($cm)) {
            $data = new \stdClass();
            $data->foreact = $foreact;
            throw new \core\message\inbound\processing_failed_exception('messageinboundforeacthidden', 'mod_foreact', $data);
        }
        ...
        if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundthresholdhit', 'mod_foreact', $data);
        }

        $subject = clean_param($messagedata->envelope->subject, PARAM_TEXT);
        $restring = get_string('re', 'foreact');
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (!foreact_can_create_attachment($foreact, $modcontext)) {
                ...
                mtrace("--> User does not have permission to attach files in this foreact. Rejecting e-mail.");

                $data = new \stdClass();
                $data->foreact = $foreact;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundattachmentdisallowed', 'mod_foreact', $data);
            }

            if ($foreact->maxattachments < $attachmentcount) {
                // Too many attachments.
                mtrace("--> User attached {$attachmentcount} files when only {$foreact->maxattachments} where allowed. "
                     . " Rejecting e-mail.");

                $data = new \stdClass();
                $data->foreact = $foreact;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilecountexceeded', 'mod_foreact', $data);
            }
            ...
            if ($foreact->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$foreact->maxbytes} where allowed. "
                     . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->foreact = $foreact;
                $data->maxbytes = display_size($foreact->maxbytes);
                $data->filesize = display_size($filesize);
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilesizeexceeded', 'mod_foreact', $data);
            }
        }
        ...
        $addpost->id = foreact_add_new_post($addpost, true);
        ...
        $event->add_record_snapshot('foreact_posts', $addpost);
        $event->add_record_snapshot('foreact_discussions', $discussion);
        $event->trigger();

        // Update completion state.
        $completion = new \completion_info($course);
        if ($completion->is_enabled($cm) && ($foreact->completionreplies || $foreact->completionposts)) {
            $completion->update_state($cm, COMPLETION_COMPLETE);

            mtrace("--> Updating completion status for user {$USER->id} in foreact {$foreact->id} for post {$addpost->id}.");
        }
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/foreact/discuss.php', array('d' => $handlerresult->discussion));
        $discussionurl->set_anchor('p' . $handlerresult->id);
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_foreact', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_foreact', $a);
        return $message;
    }
}
