 ...
 */
class foreact_post implements \renderable, \templatable {

    /**
     * The course that the foreact post is in.
     *
     * @var object $course
     */
    ...
    /**
     * The course module for the foreact.
     *
     * @var object $cm
     */
    protected $cm = null;
    ...
    /**
     * The foreact that the post is in.
     *
     * @var object $foreact
     */
    protected $foreact = null;

    /**
     * The discussion that the foreact post is in.
     *
    ...
    /**
     * The foreact post being displayed.
     *
     * @var object $post
     */
    protected $post = null;
    ...
    /**
     * Whether to override foreact display when displaying usernames.
    ...
    /**
     * Builds a renderable foreact post
     *
     * @param object $course Course of the foreact
     * @param object $cm Course Module of the foreact
     * @param object $foreact The foreact of the post
     ...
     */
    public function __construct($course, $cm, $foreact, $discussion, $post, $author, $recipient, $canreply) {
        $this->course = $course;
        $this->cm = $cm;
     ...
     *
     * @param \mod_foreact_renderer $renderer The render to be used for formatting the message and attachments
     ...
     * @return stdClass Data ready for use in a mustache template
     */
    protected function export_for_template_text(\mod_foreact_renderer $renderer) {
        return array(
            ...
            'foreactname'                     => html_entity_decode($this->get_foreactname()),
            ...
            'unsubscribeforeactlink'          => $this->get_unsubscribeforeactlink(),
            'parentpostlink'                => $this->get_parentpostlink(),

            'foreactindexlink'                => $this->get_foreactindexlink(),
            'foreactviewlink'                 => $this->get_foreactviewlink(),
     ...
     * @return stdClass Data ready for use in a mustache template
     */
    protected function export_for_template_html(\mod_foreact_renderer $renderer) {
        return array(
            'id'                            => $this->post->id,
            'coursename'                    => $this->get_coursename(),
            'courselink'                    => $this->get_courselink(),
            'foreactname'                     => $this->get_foreactname(),
    ...
    /**
     * Get the link to the foreact index for this course.
     *
     * @return string
     */
    public function get_foreactindexlink() {
        $link = new \moodle_url(
            // Posts are viewed on the topic.
            '/mod/foreact/index.php', array(
                'id'    => $this->course->id,
            )
        );
    ...
    /**
     * Get the link to the view page for this foreact.
     *
     * @return string
     */
    public function get_foreactviewlink() {
        $link = new \moodle_url(
            // Posts are viewed on the topic.
            '/mod/foreact/view.php', array(
                'f' => $this->foreact->id,
            )
        );
    ...
    /**
     * Get the link to unsubscribe from the foreact.
     *
     * @return string
     */
    public function get_unsubscribeforeactlink() {
        if (!\mod_foreact\subscriptions::is_subscribable($this->foreact)) {
            return null;
        }
        $link = new \moodle_url(
            '/mod/foreact/subscribe.php', array(
                'id' => $this->foreact->id,
            )
        );
    ...
    /**
     * ID number of the course that the foreact is in.
     *
     * @return string
     */
    ...
    /**
     * The full name of the course that the foreact is in.
     *
     * @return string
     */
    ...
    /**
     * The name of the course that the foreact is in.
     *
     * @return string
     */
    ...
    /**
     * The name of the foreact.
     *
     * @return string
     */
    public function get_foreactname() {
        return format_string($this->foreact->name, true);
    }
     ...
     * If the foreact name matches the discussion name, the discussion name
     * is not typically displayed.
     *
     * @return boolean
     */
    public function get_showdiscussionname() {
        return ($this->foreact->name !== $this->discussion->name);
    }
    ...
    public function get_postdate() {
        ...
        if (!empty($CFG->foreact_enabletimedposts) && ($this->discussion->timestart > $postmodified)) {
    ...
    public function get_group_picture() {
        if (isset($this->userfrom->groups)) {
            $groups = $this->userfrom->groups[$this->foreact->id];
        } else {
