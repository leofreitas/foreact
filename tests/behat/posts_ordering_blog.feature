...
Feature: Blog posts are always displayed in reverse chronological order
  In order to use foreact as a blog
  As a user
  ...
  Background:
    ...
    And I add a "foreact" to section "1" and I fill the form with:
      | foreact name  | Course blog foreact                               |
      | Description | Single discussion foreact description             |
      | foreact type  | Standard foreact displayed in a blog-like format  |
    And I log out
  ...
  Scenario: Replying to a blog post or editing it does not affect its display order
    ...
    And I follow "Course blog foreact"
    #
    # Add three posts into the blog.
    #
    When I add a new topic to "Course blog foreact" foreact with:
      | Subject | Blog post 1             |
      | Message | This is the first post  |
    And I add a new topic to "Course blog foreact" foreact with:
      | Subject | Blog post 2             |
    ...
    #
    Then I should see "This is the third post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][position()=1]" "xpath_element"
    And I should see "This is the second post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][position()=2]" "xpath_element"
    And I should see "This is the first post" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' foreactpost ')][position()=3]" "xpath_element"
    #
