...
/**
 * Library of functions for foreact outside of the core api
 */
 ...
 */
class foreact_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'foreact');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('foreact_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'foreact');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('foreact_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'foreact');
        }

        if (!$this->foreact = $DB->get_record('foreact', array('id' => $this->discussion->foreact))) {
            throw new portfolio_caller_exception('invalidforeactid', 'foreact');
        }

        if (!$this->cm = get_coursemodule_from_instance('foreact', $this->foreact->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_foreact', 'attachment', $this->post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_foreact', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = foreact_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_foreact', 'attachment', $post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_foreact', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/foreact/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/foreact/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('foreactdiscussion' . $this->discussionid,
                    get_string('discussion', 'foreact') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single foreact post,
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('foreactpost' . $post->id,  $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id])) {
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'foreactpost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in foreact_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_foreact', 'post', $post->id, $format);

        $output = '<table border="0" cellpadding="3" cellspacing="0" class="foreactpost">';
        ...
        $output .= '<div class="author">'.get_string('bynameondate', 'foreact', $by).'</div>';
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'foreact') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/foreact:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/foreact:exportownpost', $context)));
        }
        return has_capability('mod/foreact:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'foreact');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class foreact_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_foreact', $this->filearea, $record->itemid))
...
/**
 * Returns foreact posts tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_foreact/foreact_posts to search for foreact posts
 ...
 */
function mod_foreact_get_tagged_posts($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = 1, $page = 0) {
    global $OUTPUT;
    $perpage = $exclusivemode ? 20 : 5;

    // Build the SQL query.
    $ctxselect = context_helper::get_preload_record_columns_sql('ctx');
    $query = "SELECT fp.id, fp.subject, fd.foreact, fp.discussion, f.type, fd.timestart, fd.timeend, fd.groupid, fd.firstpost,
                    fp.parent, fp.userid,
                    cm.id AS cmid, c.id AS courseid, c.shortname, c.fullname, $ctxselect
                FROM {foreact_posts} fp
                JOIN {foreact_discussions} fd ON fp.discussion = fd.id
                JOIN {foreact} f ON f.id = fd.foreact
                JOIN {modules} m ON m.name='foreact'
    ...
    $params = array('itemtype' => 'foreact_posts', 'tagid' => $tag->id, 'component' => 'mod_foreact',
    ...
    $builder = new core_tag_index_builder('mod_foreact', 'foreact_posts', $query, $params, $page * $perpage, $perpage + 1);
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        $builder->walk(function ($taggeditem) use ($courseid, $modinfo, $builder, $item) {
            // Checking permission for Q&A foreacts performs additional DB queries, do not do them in bulk.
            if ($taggeditem->courseid == $courseid && ($taggeditem->type != 'qanda' || $taggeditem->id == $item->id)) {
                $cm = $modinfo->get_cm($taggeditem->cmid);
                $foreact = (object)['id'     => $taggeditem->foreact,
                ...
                $accessible = foreact_user_can_see_post($foreact, $discussion, $post, null, $cm);
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $pageurl = new moodle_url('/mod/foreact/discuss.php', array('d' => $item->discussion), 'p' . $item->id);
        ...
        return new core_tag\output\tagindex($tag, 'mod_foreact', 'foreact_posts', $content,
