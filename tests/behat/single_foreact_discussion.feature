@mod @mod_foreact
Feature: Single simple foreact discussion type
  In order to restrict the discussion topic to one
  As a teacher
  I need to create a foreact with a single simple discussion
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | foreact      | Single discussion foreact name | Single discussion foreact description | single  | C1     | foreact        |

  Scenario: Teacher can start the single simple discussion
    Given I log in as "teacher1"
    And I am on "Course 1" course homepage
    When I follow "Single discussion foreact name"
    Then I should see "Single discussion foreact description" in the "div.firstpost.starter" "css_element"
    And I should not see "Add a new discussion topic"

  Scenario: Student can not add more discussions
    And I log in as "student1"
    And I am on "Course 1" course homepage
    When I reply "Single discussion foreact name" post from "Single discussion foreact name" foreact with:
