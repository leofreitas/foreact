...
Feature: Posting to groups in a separate group discussion when restricted to groupings
  In order to post to groups in a foreact with separate groups and groupings
  As a teacher
  ...
  Background:
    ...
    And I add a "foreact" to section "1" and I fill the form with:
      | foreact name  | Multiple groups foreact             |
      | foreact type  | Standard foreact for general use    |
      | Description | Standard foreact description        |
      | Group mode  | Separate groups                   |
      | Grouping    | G1                                |
    And I add a "foreact" to section "1" and I fill the form with:
      | foreact name  | Single groups foreact               |
      | foreact type  | Standard foreact for general use    |
      | Description | Standard foreact description        |
      | Group mode  | Separate groups                   |
      | Grouping    | G2                                |
  ...
  Scenario: Teacher with accessallgroups can post in all groups
    ...
    And I follow "Multiple groups foreact"
    ...
    And I am on "Course 1" course homepage
    And I follow "Single groups foreact"
