...
$foreactid = trim(optional_param('foreactid', 0, PARAM_INT));      // foreactID to search for
...
if (empty($search)) {   // Check the other parameters instead
    ...
    if (!empty($foreactid)) {
        $search .= ' foreactid:'.$foreactid;
    }
    if (!empty($user)) {
        $search .= ' '.foreact_clean_search_terms($user, 'user:');
    }
    if (!empty($subject)) {
        $search .= ' '.foreact_clean_search_terms($subject, 'subject:');
    }
    if (!empty($fullwords)) {
        $search .= ' '.foreact_clean_search_terms($fullwords, '+');
    }
    if (!empty($notwords)) {
        $search .= ' '.foreact_clean_search_terms($notwords, '-');
    }
    if (!empty($phrase)) {
        $search .= ' "'.$phrase.'"';
    }
...
if ($search) {
    $search = foreact_clean_search_terms($search);
}
...
$strsearch = get_string("search", "foreact");
$strsearchresults = get_string("searchresults", "foreact");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strforeacts, new moodle_url('/mod/foreact/index.php', array('id'=>$course->id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'foreact'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    foreact_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchterms = str_replace('foreactid:', 'instance:', $search);
$searchterms = explode(' ', $searchterms);

$searchform = foreact_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/mod/foreact/search.php', array('id'=>$course->id)));
$PAGE->navbar->add($strsearchresults);
if (!$posts = foreact_search_posts($searchterms, $course->id, $page*$perpage, $perpage, $totalcount)) {
    $PAGE->set_title($strsearchresults);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading($strforeacts, 2);
    echo $OUTPUT->heading($strsearchresults, 3);
    echo $OUTPUT->heading(get_string("noposts", "foreact"), 4);
...
$url    = new moodle_url("/mod/foreact/search.php", $params);
foreach ($tags as $tag) {
    $url .= "&tags[]=$tag";
}
echo html_writer::link($url, get_string('advancedsearch', 'foreact').'...');
...
foreach ($posts as $post) {
    ...
    // Replace the simple subject with the three items foreact name -> thread name -> subject
    ...
    $fullsubject = "<a href=\"view.php?f=$foreact->id\">".format_string($foreact->name,true)."</a>";
    ...
    //Unfortunately seem to have do this individually as posts may be from different foreacts
    ...
    // the start of the message text. However, do not do the highlighting here. foreact_print_post
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','foreact');
        $post->message = '<p class="highlight2">'.$strmissingsearchterms.' '.$missing_terms.'</p>'.$post->message;
    }

    // Prepare a link to the post in context, to be displayed after the foreact post.
    $fulllink = "<a href=\"discuss.php?d=$post->discussion#p$post->id\">".get_string("postincontext", "foreact")."</a>";
...
echo $OUTPUT->footer();
  ...
  */
function foreact_print_big_search_form($course) {
    global $PAGE, $words, $subject, $phrase, $user, $fullwords, $notwords, $datefrom, $dateto, $foreactid, $tags;

    $renderable = new \mod_foreact\output\big_search_form($course, $user);
    ...
    $renderable->set_foreactid($foreactid);
 ...
 */
function foreact_clean_search_terms($words, $prefix='') {
 ...
 /**
  * Retrieve a list of the foreacts that this user can view.
  *
  * @param stdClass $course The Course to use.
  * @return array A set of formatted foreact names stored against the foreact id.
  */
function foreact_menu_list($course)  {
    $menu = array();
    ...
    foreach ($modinfo->instances['foreact'] as $cm) {
        if (!$cm->uservisible) {
            continue;
        }
