...
class mod_foreact_external_testcase extends externallib_advanced_testcase {
    ...
    protected function setUp() {
        ...
        require_once($CFG->dirroot . '/mod/foreact/externallib.php');
    }
     ...
     * Test get foreacts
     */
    public function test_mod_foreact_get_foreacts_by_courses() {
        global $USER, $CFG, $DB;

        $this->resetAfterTest(true);

        // Create a user.
        $user = self::getDataGenerator()->create_user(array('trackforeacts' => 1));
        ...
        $record->trackingtype = foreact_TRACKING_FORCED;
        $foreact1 = self::getDataGenerator()->create_module('foreact', $record);

        // Second foreact.
        $record = new stdClass();
        $record->introformat = FORMAT_HTML;
        $record->course = $course2->id;
        $record->trackingtype = foreact_TRACKING_OFF;
        $foreact2 = self::getDataGenerator()->create_module('foreact', $record);
        $foreact2->introfiles = [];

        // Add discussions to the foreacts.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->foreact = $foreact1->id;
        $discussion1 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_discussion($record);
        // Expect one discussion.
        $foreact1->numdiscussions = 1;
        $foreact1->cancreatediscussions = true;
        $foreact1->istracked = true;
        $foreact1->unreadpostscount = 0;
        $foreact1->introfiles = [];
        ...
        $discussion2 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_discussion($record);
        $discussion3 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_discussion($record);
        // Expect two discussions.
        $foreact2->numdiscussions = 2;
        // Default limited role, no create discussion capability enabled.
        $foreact2->cancreatediscussions = false;
        $foreact2->istracked = false;

        // Check the foreact was correctly created.
        $this->assertEquals(2, $DB->count_records_select('foreact', 'id = :foreact1 OR id = :foreact2',
                array('foreact1' => $foreact1->id, 'foreact2' => $foreact2->id)));
        ...
        // Assign capabilities to view foreacts for foreact 2.
        $cm2 = get_coursemodule_from_id('foreact', $foreact2->cmid, 0, false, MUST_EXIST);
        $context2 = context_module::instance($cm2->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $roleid2 = $this->assignUserCapability('mod/foreact:viewdiscussion', $context2->id, $newrole);

        // Create what we expect to be returned when querying the two courses.
        unset($foreact1->displaywordcount);
        unset($foreact2->displaywordcount);

        $expectedforeacts = array();
        $expectedforeacts[$foreact1->id] = (array) $foreact1;
        $expectedforeacts[$foreact2->id] = (array) $foreact2;

        // Call the external function passing course ids.
        $foreacts = mod_foreact_external::get_foreacts_by_courses(array($course1->id, $course2->id));
        $foreacts = external_api::clean_returnvalue(mod_foreact_external::get_foreacts_by_courses_returns(), $foreacts);
        $this->assertCount(2, $foreacts);
        foreach ($foreacts as $foreact) {
            $this->assertEquals($expectedforeacts[$foreact['id']], $foreact);
        }

        // Call the external function without passing course id.
        $foreacts = mod_foreact_external::get_foreacts_by_courses();
        $foreacts = external_api::clean_returnvalue(mod_foreact_external::get_foreacts_by_courses_returns(), $foreacts);
        $this->assertCount(2, $foreacts);
        foreach ($foreacts as $foreact) {
            $this->assertEquals($expectedforeacts[$foreact['id']], $foreact);
        ...
        // Unenrol user from second course and alter expected foreacts.
        $enrol->unenrol_user($instance2, $user->id);
        unset($expectedforeacts[$foreact2->id]);

        // Call the external function without passing course id.
        $foreacts = mod_foreact_external::get_foreacts_by_courses();
        $foreacts = external_api::clean_returnvalue(mod_foreact_external::get_foreacts_by_courses_returns(), $foreacts);
        $this->assertCount(1, $foreacts);
        $this->assertEquals($expectedforeacts[$foreact1->id], $foreacts[0]);
        $this->assertTrue($foreacts[0]['cancreatediscussions']);

        // Change the type of the foreact, the user shouldn't be able to add discussions.
        $DB->set_field('foreact', 'type', 'news', array('id' => $foreact1->id));
        $foreacts = mod_foreact_external::get_foreacts_by_courses();
        $foreacts = external_api::clean_returnvalue(mod_foreact_external::get_foreacts_by_courses_returns(), $foreacts);
        $this->assertFalse($foreacts[0]['cancreatediscussions']);

        // Call for the second course we unenrolled the user from.
        $foreacts = mod_foreact_external::get_foreacts_by_courses(array($course2->id));
        $foreacts = external_api::clean_returnvalue(mod_foreact_external::get_foreacts_by_courses_returns(), $foreacts);
        $this->assertCount(0, $foreacts);
    }

    /**
     * Test get foreact posts
     */
    public function test_mod_foreact_get_foreact_discussion_posts() {
        global $CFG, $PAGE;

        $this->resetAfterTest(true);

        // Set the CFG variable to allow track foreacts.
        $CFG->foreact_trackreadposts = true;

        // Create a user who can track foreacts.
        $record = new stdClass();
        $record->trackforeacts = true;
        ...
        // foreact with tracking off.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->trackingtype = foreact_TRACKING_OFF;
        $foreact1 = self::getDataGenerator()->create_module('foreact', $record);
        $foreact1context = context_module::instance($foreact1->cmid);

        // foreact with tracking enabled.
        $record = new stdClass();
        $record->course = $course1->id;
        $foreact2 = self::getDataGenerator()->create_module('foreact', $record);
        $foreact2cm = get_coursemodule_from_id('foreact', $foreact2->cmid);
        $foreact2context = context_module::instance($foreact2->cmid);
        ...
        $record->userid = $user2->id;
        $discussion1reply1 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_post($record);
        $filename = 'shouldbeanimage.jpg';
        // Add a fake inline image to the post.
        $filerecordinline = array(
            'contextid' => $foreact1context->id,
        ...
        $discussion1reply2 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_post($record);

        // Enrol the user in the  course.
        $enrol = enrol_get_plugin('manual');
        // Following line enrol and assign default role id to the user.
        // So the user automatically gets mod/foreact:viewdiscussion on all foreacts of the course.
        ...
        $expectedposts['posts'][] = array(
            ...
            'message' => file_rewrite_pluginfile_urls($discussion1reply2->message, 'pluginfile.php',
                    $foreact1context->id, 'mod_foreact', 'post', $discussion1reply2->id),
        ...
        $expectedposts['posts'][] = array(
            ...
            'message' => file_rewrite_pluginfile_urls($discussion1reply1->message, 'pluginfile.php',
                    $foreact1context->id, 'mod_foreact', 'post', $discussion1reply1->id),
            'messageformat' => 1,   // This value is usually changed by external_format_text() function.
            ...
            'messageinlinefiles' => array(
                array(
                    ...
                    'filepath' => '/',
                    'filesize' => '27',
                    'fileurl' => moodle_url::make_webservice_pluginfile_url($foreact1context->id, 'mod_foreact', 'post',
        ...
        $posts = mod_foreact_external::get_foreact_discussion_posts($discussion1->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussion_posts_returns(), $posts);
        ...
        // Check we receive the unread count correctly on tracked foreact.
        foreact_tp_count_foreact_unread_posts($foreact2cm, $course1, true);    // Reset static cache.
        $result = mod_foreact_external::get_foreacts_by_courses(array($course1->id));
        $result = external_api::clean_returnvalue(mod_foreact_external::get_foreacts_by_courses_returns(), $result);
        foreach ($result as $f) {
            if ($f['id'] == $foreact2->id) {
        ...
        $posts = mod_foreact_external::get_foreact_discussion_posts($discussion2->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussion_posts_returns(), $posts);
        $this->assertEquals(1, count($posts['posts']));

        // Test discussion tracking on not tracked foreact.
        $result = mod_foreact_external::view_foreact_discussion($discussion1->id);
        $result = external_api::clean_returnvalue(mod_foreact_external::view_foreact_discussion_returns(), $result);
        ...
        // Test discussion tracking on tracked foreact.
        $result = mod_foreact_external::view_foreact_discussion($discussion3->id);
        $result = external_api::clean_returnvalue(mod_foreact_external::view_foreact_discussion_returns(), $result);
        $this->assertTrue($result['status']);
        $this->assertEmpty($result['warnings']);

        // Test posts have been marked as read.
        $posts = mod_foreact_external::get_foreact_discussion_posts($discussion3->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussion_posts_returns(), $posts);
        foreach ($posts['posts'] as $post) {
    ...
    /**
     * Test get foreact posts (qanda foreact)
     */
    public function test_mod_foreact_get_foreact_discussion_posts_qanda() {
        global $CFG, $DB;
        ...
        $DB->update_record('foreact_posts', $discussion1reply2);
    ...
    /**
     * Test get foreact discussions paginated
     */
    public function test_mod_foreact_get_foreact_discussions_paginated() {
        ...
        // First foreact with tracking off.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->trackingtype = foreact_TRACKING_OFF;
        $foreact1 = self::getDataGenerator()->create_module('foreact', $record);
        ...
        $discussion1reply3 = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_post($record);
        ...
        // Assign capabilities to view discussions for foreact 1.
        $cm = get_coursemodule_from_id('foreact', $foreact1->cmid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('mod/foreact:viewdiscussion', $context->id, $newrole);

        // Create what we expect to be returned when querying the foreacts.

        $post1 = $DB->get_record('foreact_posts', array('id' => $discussion1->firstpost), '*', MUST_EXIST);
        ...
        $expecteddiscussions = array(
                ...
                'numreplies' => 3,
                'numunread' => 0,
                'pinned' => foreact_DISCUSSION_UNPINNED,
                'locked' => false,
                'canreply' => false,
            );

        // Call the external function passing foreact id.
        $discussions = mod_foreact_external::get_foreact_discussions_paginated($foreact1->id);
        $discussions = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussions_paginated_returns(), $discussions);
        ...
        $this->unassignUserCapability('mod/foreact:viewdiscussion', $context->id, $newrole);
        try {
            mod_foreact_external::get_foreact_discussions_paginated($foreact1->id);
    ...
    /**
     * Test get foreact discussions paginated (qanda foreacts)
     */
    public function test_mod_foreact_get_foreact_discussions_paginated_qanda() {
        ...
        $record->type = 'qanda';
        $foreact = self::getDataGenerator()->create_module('foreact', $record);

        // Add discussions to the foreacts.
        $discussionrecord = new stdClass();
        $discussionrecord->course = $course->id;
        $discussionrecord->userid = $user2->id;
        $discussionrecord->foreact = $foreact->id;
        $discussion = self::getDataGenerator()->get_plugin_generator('mod_foreact')->create_discussion($discussionrecord);

        self::setAdminUser();
        $discussions = mod_foreact_external::get_foreact_discussions_paginated($foreact->id);
        $discussions = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussions_paginated_returns(), $discussions);
    ...
    public function test_add_discussion_post() {
        ...
        $record->course = $course->id;
        $foreact = self::getDataGenerator()->create_module('foreact', $record);
        $cm = get_coursemodule_from_id('foreact', $foreact->cmid, 0, false, MUST_EXIST);
        $foreactcontext = context_module::instance($foreact->cmid);
        ...
        self::setUser($user);
        try {
            mod_foreact_external::add_discussion_post($discussion->firstpost, 'some subject', 'some text here...');
            $this->fail('Exception expected due to being unenrolled from the course.');
        } catch (moodle_exception $e) {
            $this->assertEquals('requireloginerror', $e->errorcode);
        }
        ...
        $createdpost = mod_foreact_external::add_discussion_post($discussion->firstpost, 'some subject', 'some text here...');
        $createdpost = external_api::clean_returnvalue(mod_foreact_external::add_discussion_post_returns(), $createdpost);

        $posts = mod_foreact_external::get_foreact_discussion_posts($discussion->id);
        $posts = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussion_posts_returns(), $posts);
        ...
        $dummytext = 'Here is an inline image: <img src="' . $CFG->wwwroot
                     ...
                     . '" alt="inlineimage">.';
        $createdpost = mod_foreact_external::add_discussion_post($discussion->firstpost, 'new post inline attachment',
                                                               $dummytext, $options);
        $createdpost = external_api::clean_returnvalue(mod_foreact_external::add_discussion_post_returns(), $createdpost);
        ...
        try {
            ...
            $this->assertEquals('nopostforeact', $e->errorcode);
        }
    ...
    public function test_add_discussion() {
        ...
        try {
            mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...');
        ...
        self::setAdminUser();
        $createddiscussion = mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...');
        $createddiscussion = external_api::clean_returnvalue(mod_foreact_external::add_discussion_returns(), $createddiscussion);
        ...
        $discussion2pinned = mod_foreact_external::add_discussion($foreact->id, 'the pinned subject', 'some 2 text here...', -1,
                                                                array('options' => array('name' => 'discussionpinned',
                                                                                         'value' => true)));
        $discussion3 = mod_foreact_external::add_discussion($foreact->id, 'the non pinnedsubject', 'some 3 text here...');
        $discussions = mod_foreact_external::get_foreact_discussions_paginated($foreact->id);
        $discussions = external_api::clean_returnvalue(mod_foreact_external::get_foreact_discussions_paginated_returns(), $discussions);
        ...
        $options = array(array('name' => 'inlineattachmentsid', 'value' => $draftidinlineattach),
                         array('name' => 'attachmentsid', 'value' => $draftidattach));
        $createddiscussion = mod_foreact_external::add_discussion($foreact->id, 'the inline attachment subject',
                                                                $dummytext, -1, $options);
        $createddiscussion = external_api::clean_returnvalue(mod_foreact_external::add_discussion_returns(), $createddiscussion);
    ...
    public function test_add_discussion_in_course_with_groups() {
        ...
        try {
            mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...', 0);
            $this->fail('Exception expected due to invalid group permissions.');
        } catch (moodle_exception $e) {
            $this->assertEquals('cannotcreatediscussion', $e->errorcode);
        }
        ...
        try {
            mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...', $group->id);
            $this->fail('Exception expected due to invalid group permissions.');
        } catch (moodle_exception $e) {
            $this->assertEquals('cannotcreatediscussion', $e->errorcode);
        }
        ...
        // Try to post in a group the user is not enrolled.
        try {
            mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...', $group->id + 1);
        ...
        $discussion = mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...', $group->id);
        $discussion = external_api::clean_returnvalue(mod_foreact_external::add_discussion_returns(), $discussion);
        ...
        $discussion = mod_foreact_external::add_discussion($foreact->id, 'the subject', 'some text here...');
        $discussion = external_api::clean_returnvalue(mod_foreact_external::add_discussion_returns(), $discussion);
    ...
    public function test_can_add_discussion() {
        ...
        // User with no permissions to add in a news foreact.
        self::setUser($user);
        $this->getDataGenerator()->enrol_user($user->id, $course->id);

        $result = mod_foreact_external::can_add_discussion($foreact->id);
        $result = external_api::clean_returnvalue(mod_foreact_external::can_add_discussion_returns(), $result);
        $this->assertFalse($result['status']);
        $this->assertFalse($result['canpindiscussions']);
        $this->assertTrue($result['cancreateattachment']);

        // Disable attachments.
        $DB->set_field('foreact', 'maxattachments', 0, array('id' => $foreact->id));
        $result = mod_foreact_external::can_add_discussion($foreact->id);
        $result = external_api::clean_returnvalue(mod_foreact_external::can_add_discussion_returns(), $result);
        $this->assertFalse($result['status']);
        $this->assertFalse($result['canpindiscussions']);
        $this->assertFalse($result['cancreateattachment']);
        $DB->set_field('foreact', 'maxattachments', 1, array('id' => $foreact->id));    // Enable attachments again.
