...
/**
 * The foreact module mail generation tests.
 *
 * @package    mod_foreact
 * @category   external
...
class mod_foreact_mail_testcase extends advanced_testcase {
    ...
    /**
     * Perform message inbound setup for the mod_foreact reply handler.
     */
    protected function helper_spoof_message_inbound_setup() {
        ...
        $record = $DB->get_record('messageinbound_handlers', array('classname' => '\mod_foreact\message\inbound\reply_handler'));
        $record->enabled = true;
     ...
     */
    protected function helper_post_to_foreact($foreact, $author, $fields = array()) {
        global $DB;
    ...
    protected function helper_update_post_time($post, $factor) {
        ...
        $DB->set_field('foreact_posts', 'created', $post->created + $factor, array('id' => $post->id));
    }
    ...
    protected function helper_update_subscription_time($user, $discussion, $factor) {
        ...
        $sub = $DB->get_record('foreact_discussion_subs', array('userid' => $user->id, 'discussion' => $discussion->id));

        // Update the subscription to have a preference in the past.
        $DB->set_field('foreact_discussion_subs', 'preference', $sub->preference + $factor, array('id' => $sub->id));
    }
     ...
     * @param stdClass $author The author to post as
     * @return stdClass The foreact post
     */
    protected function helper_post_to_discussion($foreact, $discussion, $author) {
        global $DB;
     ...
     *
     * @param stdClass $post The foreact post object
    ...
    /**
     * Run the foreact cron, and check that the specified posts were sent the
     * specified number of times.
     *
     * @param stdClass $post The foreact post object
     * @param integer $expected The number of times that the post should have been sent
    ...
    public function test_subscription_disabled() {
        ...
        \mod_foreact\subscriptions::subscribe_user($recipient->id, $foreact);

        $this->assertEquals($expected, $DB->count_records('foreact_subscriptions', array(
            'userid'        => $recipient->id,
            'foreact'         => $foreact->id,
        )));

        // Run cron and check that the expected number of users received the notification.
        list($discussion, $post) = $this->helper_post_to_foreact($foreact, $recipient);
        $messages = $this->helper_run_cron_check_count($post, $expected);

        // Unsubscribe the user again.
        \mod_foreact\subscriptions::unsubscribe_user($recipient->id, $foreact);
        ...
        \mod_foreact\subscriptions::subscribe_user_to_discussion($recipient->id, $discussion);
        $this->helper_update_subscription_time($recipient, $discussion, -60);

        $reply = $this->helper_post_to_discussion($foreact, $discussion, $author);
    ...
    public function test_automatic_with_unsubscribed_user() {
        ...
        // Unsubscribe the 'author' user from the foreact.
    ...
    public function test_optional_with_subscribed_user() {
        ...
        // Subscribe the 'recipient' user from the foreact.
        \mod_foreact\subscriptions::subscribe_user($recipient->id, $foreact);
    ...
    public function test_automatic_with_unsubscribed_discussion() {
        ...
        $this->assertTrue(\mod_foreact\subscriptions::is_subscribed($recipient->id, $foreact, $discussion->id));
    ...
    public function test_automatic_with_subscribed_discussion_in_unsubscribed_foreact() {
        $this->resetAfterTest(true);
        ...
        // We expect just the user subscribed to the foreact to receive this post at the moment as the discussion
    ...
    public function test_optional_with_unsubscribed_discussion_in_subscribed_foreact() {
        $this->resetAfterTest(true);
        ...
        \mod_foreact\subscriptions::unsubscribe_user_from_discussion($recipient->id, $discussion);
    ...
    /**
     * Test that a user unsubscribed from a foreact who has subscribed to a discussion, only receives posts made after
     * they subscribed to the discussion.
     */
    public function test_foreact_discussion_subscription_foreact_unsubscribed_discussion_subscribed_after_post() {
        $this->resetAfterTest(true);
    ...
    public function test_foreact_message_inbound_multiple_posts() {
        $this->resetAfterTest(true);
        ...
        // Ensure that messageinbound is enabled and configured for the foreact handler.
        $this->helper_spoof_message_inbound_setup();

        $author->emailstop = '0';
        set_user_preference('message_provider_mod_foreact_posts_loggedoff', 'email', $author);
        set_user_preference('message_provider_mod_foreact_posts_loggedin', 'email', $author);
    ...
    public function test_long_subject() {
        ...
        // Post a discussion to the foreact.
        $subject = 'This is the very long foreact post subject that somebody was very kind of leaving, it is intended to check if long subject comes in mail correctly. Thank you.';
        $a = (object)array('courseshortname' => $course->shortname, 'foreactname' => $foreact->name, 'subject' => $subject);
        $expectedsubject = get_string('postmailsubject', 'foreact', $a);
        list($discussion, $post) = $this->helper_post_to_foreact($foreact, $author, array('name' => $subject));
    ...
    public function test_subjects() {
        ...
        $reply = $this->helper_post_to_discussion($foreact, $discussion, $commenter);
        $messages = $this->helper_run_cron_check_count($reply, 2);
        $this->assertContains($strre, $messages[0]->subject);
    }

    /**
     * dataProvider for test_foreact_post_email_templates().
     */
    public function foreact_post_email_templates_provider() {
                ...
                array(
                    ...
                    'contents' => array(
                        ...
                        'Attachment example.txt:\n' .
                            'https://www.example.com/moodle/pluginfile.php/\d*/mod_foreact/attachment/\d*/example.txt\n',
                        'Hello Moodle', 'Moodle foreact', 'Welcome.*Moodle', 'Love Moodle', '1\d1'
                    ),
                ),
            ),
        );
        ...
        $newcase['foreacts'][0]['name'] = 'Moodle foreact\'"';
        $newcase['foreacts'][0]['foreactposts'][0]['name'] = 'Hello Moodle\'"';
        $newcase['foreacts'][0]['foreactposts'][0]['message'] = 'Welcome to Moodle\'"';
        $newcase['expectations'][0]['contents'] = array(
            'Attachment example.txt:', '~{\$a', '~&amp;(quot|\#039);', 'Love Moodle\'', '101\'', 'Moodle foreact\'"',
        ...
        $newcase['foreacts'][0]['foreactposts'][0]['message'] = 'Welcome to Moodle>';
        $newcase['expectations'][0]['contents'] = array(
            'Attachment example.txt:', '~{\$a', '~&amp;gt;', 'Love Moodle>', '101>', 'Moodle foreact>',
            'Hello Moodle>', 'Welcome to Moodle>');
        ...
        $newcase = $base;
        $newcase['foreacts'][0]['foreactposts'][0]['name'] = 'Text and image';
        $newcase['foreacts'][0]['foreactposts'][0]['message'] = 'Welcome to Moodle, '
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            'Attachment example.txt:\n' .
            'https://www.example.com/moodle/pluginfile.php/\d*/mod_foreact/attachment/\d*/example.txt\n',
            'Text and image', 'Moodle foreact',
            'Welcome to Moodle, *\n.*'
                .'https://www.example.com/moodle/pluginfile.php/\d+/mod_foreact/post/\d+/'
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            '~&amp;(amp|lt|gt|quot|\#039);',
            '<div class="attachments">( *\n *)?<a href',
            '<div class="subject">\n.*Hello Moodle\'"&gt;&amp;', '>Moodle foreact\'"&gt;&amp;',
            '>Welcome.*Moodle\'"&gt;&amp;', '>Love Moodle&\#039;&quot;&gt;&amp;', '>101\'"&gt;&amp');
        $htmlcases['HTML mail with quotes, gt, lt and ampersand  everywhere'] = array('data' => $newcase);

        // Text+image message i.e. @@PLUGINFILE@@ token handling.
        $newcase = $htmlbase;
        $newcase['foreacts'][0]['foreactposts'][0]['name'] = 'HTML text and image';
        $newcase['foreacts'][0]['foreactposts'][0]['message'] = '<p>Welcome to Moodle, '
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            '~&(amp|lt|gt|quot|\#039);(?!course)',
            '<div class="attachments">( *\n *)?<a href',
            '<div class="subject">\n.*HTML text and image', '>Moodle foreact',
            '<p>Welcome to Moodle, '
                .'<img src="https://www.example.com/moodle/pluginfile.php/\d+/mod_foreact/post/\d+/'
                .'Screen%20Shot%202016-03-22%20at%205\.54\.36%20AM%20%281%29\.png"'
                .' alt="" width="200" height="393" class="img-responsive" />!</p>',
            '>Love Moodle', '>1\d1');
    ...
    /**
     * Verify foreact emails body using templates to generate the expected results.
     *
     * @dataProvider foreact_post_email_templates_provider
     * @param array $data provider samples.
     */
    public function test_foreact_post_email_templates($data) {
        global $DB;
        ...
        // Create foreacts, always force susbscribed (for easy), with the specified options.
        $posts = array();
        foreach ($data['foreacts'] as $dataforeact) {
            $foreactposts = isset($dataforeact['foreactposts']) ? $dataforeact['foreactposts'] : array();
            unset($dataforeact['foreactposts']);
            $options = array('course' => $course->id, 'forcesubscribe' => foreact_FORCESUBSCRIBE);
            foreach ($dataforeact as $option => $value) {
                $options[$option] = $value;
            }
            $foreact = $this->getDataGenerator()->create_module('foreact', $options);

            // Create posts, always for immediate delivery (for easy), with the specified options.
            foreach ($foreactposts as $foreactpost) {
                $attachments = isset($foreactpost['attachments']) ? $foreactpost['attachments'] : array();
                unset($foreactpost['attachments']);
                $postoptions = array('course' => $course->id, 'foreact' => $foreact->id, 'userid' => $user->id,
                    'mailnow' => 1, 'attachment' => !empty($attachments));
                foreach ($foreactpost as $option => $value) {
                    $postoptions[$option] = $value;
                }
                list($discussion, $post) = $this->helper_post_to_foreact($foreact, $user, $postoptions);
                ...
                if ($attachments) {
                    ...
                    foreach ($attachments as $attachment) {
                        $filerecord = array(
                            'contextid' => context_module::instance($foreact->cmid)->id,
                    ...
                    $DB->set_field('foreact_posts', 'attachment', '1', array('id' => $post->id));
                }
            }
        }
        ...
        foreact_cron(); // It's really annoying that we have to run cron to test this.
