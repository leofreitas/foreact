...
$url = new moodle_url('/mod/foreact/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if ($isspecificcourse) {
    ...
    $courses = foreact_get_courses_user_posted_in($user, $discussionsonly);
}
...
$result = foreact_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_foreact');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->fullname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_foreact');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'foreact');
        } else {
            $notification = get_string('nopostsmadebyyou', 'foreact');
        }
        // These are the user's foreact interactions.
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'foreact'), new moodle_url('/mod/foreact/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'foreact'), new moodle_url('/mod/foreact/user.php',
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'foreact', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'foreact', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'foreact');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('foreact_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular foreact,
...
foreach ($result->posts as $post) {
    if (!isset($result->foreacts[$post->foreact]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $foreact = $result->foreacts[$post->foreact];
    $cm = $foreact->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $foreacturl = new moodle_url('/mod/foreact/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/foreact/discuss.php', array('d' => $post->discussion));
    ...
    if ($foreact->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($foreact->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $foreacturl;
        } else {
    ...
    $foreactname = format_string($foreact->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($foreacturl, $foreactname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $foreactname);
    }
    if ($foreact->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/foreact/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "foreact"));

    $postoutput[] = foreact_print_post($post, $discussion, $foreact, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_foreact', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_foreact', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        $pagetitle = get_string('discussionsstartedbyuserincourse', 'mod_foreact', $a);
    } else {
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_foreact', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'foreact', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'foreact'));
}
